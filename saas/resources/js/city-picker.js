...
(function(factory) {
	...
	CityPicker.prototype = {
		...
		feedText: function() {
			...
			var titleText=text.replace(/<span[^>]+>(.*?)<\/span>\/<span[^>]+>(.*?)<\/span>/g,'$1/$2');
			if (text) {
				this.$textspan.find('>.placeholder').hide();
				this.$textspan.find('>.title').html(this.getText()).show();
				this.$textspan.find('>.title').attr('title',titleText);
			} else {
		...
		feedVal: function() {
			...
			this.$element.data("code", this.getSelectCode());
		},
	...
	$.fn.citypicker = function(option) {
		if (option.dataSource) {
			ChineseDistricts = option.dataSource;
		}
